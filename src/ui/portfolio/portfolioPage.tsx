...
export const PortfolioPage: React.FC = () => {
  ...
  const selectedTag = useSelector((state: RootState) => state.portfolio.selectedTag);
  ...
  const filteredPortfolios = 
    selectedTag ?
     selectedTag == 'All Works' ? portfolios :
      portfolios.filter((portfolio) =>
        portfolio.portfolio_tags.some((tag) => tag.tag === selectedTag)
      )
      : portfolios;

  useEffect(() => {
    dispatch(getPortfolios());
    // dispatch(setSelectedTag('All Works'));
  }, [dispatch]);
  ...
  return (
    ...
    <Canvas classname="bg-greyBlueReverse">
      ...
      >
        ...
        <div className="flex flex-wrap w-full justify-center gap-10 pb-10">
          {filteredPortfolios.map(({id, coverImage, name, desc, portfolio_tags, portfolio_pictures }, index) => {
            return (
              <PortfolioCard
                key={coverImage}
                image={{
